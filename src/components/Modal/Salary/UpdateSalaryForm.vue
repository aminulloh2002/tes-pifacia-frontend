...
import { addSalary, fetchEmployeeSimplified, fetchSalaries, updateSalary } from '@/api/salaries.ts'
...
const currentSalary = computed(() => salaryStore.salaries[props.salaryIndex])
watch(
  currentSalary,
  (newEmp) => {
    if (newEmp) {
      ...
      salary.value = {
        ...
        salary: parseFloat(newEmp.salary.replace(/,/g, '')),
        payment_date: newEmp.payment_date,
        notes: newEmp.notes,
        is_paid: newEmp.is_paid ? 1 : 0,
      }
    }
  },
  { immediate: true },
...
const saveHandler = async () => {
  ...
  try {
    await updateSalary(currentSalary.value.id, salary.value)

    const salaries = await fetchSalaries()

    salaryStore.setSalaries(salaries.data)
