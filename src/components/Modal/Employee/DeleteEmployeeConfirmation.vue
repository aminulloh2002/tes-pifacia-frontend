...
import { deleteEmployee, fetchEmployees } from '@/api/employees.ts'
...
const employee = employeeStore.employees[props.employeeIndex]

const deleteHandler = async () => {
  try {
    await deleteEmployee(employee.id)
    const newEmployee = await fetchEmployees()
    if (newEmployee.status === 200) {
      employeeStore.setEmployees(newEmployee.data)
...
<template>
  <div class="mb-5">
    <h1 class="text-lg">
      ...
      <span class="font-semibold text-red-500">{{ employee.full_name }}</span> ?
    </h1>
