...
import { fetchDepartmentAudit } from '@/api/departments.ts'
import { useRoute } from 'vue-router'
import type { meta } from '@/types/types.ts'
import PaginationComponent from '@/components/PaginationComponent.vue'
import { fetchEmployeeAudit } from '@/api/employees.ts'
...
onMounted(async () => {
  const res = await fetchEmployeeAudit(route.params.id as string)
...
const fetchData = async (page: number) => {
  try {
    const response = await fetchEmployeeAudit(route.params.id as string, page)
